...
class ShortcodeCompiler
{
    ...
    public function render(array $matches): string|null|View
    {
        ...
        // Check if caching is enabled from settings, shortcode is not in ignore list, and caching is not disabled for this specific shortcode
        if (setting('shortcode_cache_enabled', false) && ! request()->ajax() && ! $this->shouldIgnoreCache($name) && $compiled->enable_caching !== 'no') {
